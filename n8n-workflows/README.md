...
| `AUTOMATION_API_KEY` | Must match `AUTOMATION_API_KEY` in GrowwMatics AI's `.env.local` |
...
- Header value: a shared secret you also store in GrowwMatics AI's `.env.local` as `N8N_WEBHOOK_SECRET`
- The GrowwMatics AI app must include `x-webhook-secret: <value>` when POSTing to this webhook URL
...
Triggered by a webhook POST from GrowwMatics AI when a new lead is created. Sends a welcome WhatsApp via Twilio, waits 3 days, checks lead status at `/api/crm/leads/{leadId}`, and sends follow-up messages at day 3 and day 7 if the lead is still active. Requires the webhook `x-webhook-secret` header for security.
...
|---|---|---|
| Where it runs | Inside GrowwMatics AI (Next.js) | External Docker container or n8n Cloud |
