    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 bg-violet-600 rounded-xl flex items-center justify-center"><ScanSearch className="w-5 h-5 text-white" /></div>
          <div>
            <h1 className="text-2xl font-bold text-slate-900">WhatsApp Report Agent</h1>
            <p className="text-sm text-slate-500">Delivers free Google Business Profile reports — connects the visitor&apos;s Google account, then sends the report card.</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
      ...
      <section className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm mb-5 space-y-3">
        ...
        <p className="text-xs text-slate-500">Tone &amp; style used to answer questions before the visitor connects their Google account.</p>
      ...
      <section className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm mb-5 space-y-3">
        <h2 className="font-bold text-slate-900">Report intro message</h2>
        <p className="text-xs text-slate-500">
          Sent immediately when a new report thread starts. Variables: {introVars.map((v) => <code key={v} className="mx-0.5 px-1 bg-white border border-slate-200 rounded">{v}</code>)}
        </p>
      ...
      <section className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm mb-6 space-y-3">
        <h2 className="font-bold text-slate-900">Report summary message</h2>
        <p className="text-xs text-slate-500">
          Sent right after the report-card image. Variables: {summaryVars.map((v) => <code key={v} className="mx-0.5 px-1 bg-white border border-slate-200 rounded">{v}</code>)}
        </p>
