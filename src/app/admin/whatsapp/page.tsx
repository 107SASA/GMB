  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="font-heading text-2xl font-bold text-on-surface">WhatsApp Monitor</h1>
          <p className="text-sm text-on-surface-variant">All inbound and outbound WhatsApp conversations — live.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Conversations', value: loading ? '—' : data?.stats.totalConversations.toLocaleString(), icon: MessageSquare, color: 'text-primary bg-primary-fixed' },
          { label: 'Active (24h)', value: loading ? '—' : data?.stats.activeThreads.toLocaleString(), icon: Zap, color: 'text-secondary bg-secondary-container/40' },
          { label: 'AI Enabled', value: loading ? '—' : data?.stats.aiEnabledThreads.toLocaleString(), icon: BrainCircuit, color: 'text-primary bg-primary-fixed' },
          { label: 'Messages Today', value: loading ? '—' : data?.stats.messagesToday.toLocaleString(), icon: Send, color: 'text-primary bg-primary-fixed' },
        ].map((card) => (
      ...
      <div className="bg-surface-container-lowest rounded-xl border border-outline-variant card-shadow p-6">
        <h2 className="font-semibold text-on-surface mb-1">Message Volume (7 days)</h2>
        <p className="text-xs text-outline mb-4">Inbound vs outbound messages per day</p>
        {loading ? (
      ...
      <div className="bg-surface-container-lowest rounded-xl border border-outline-variant card-shadow overflow-hidden">
        <div className="px-6 py-4 border-b border-outline-variant">
          <h2 className="font-semibold text-on-surface">Recent Threads</h2>
          <p className="text-xs text-outline mt-0.5">Latest 20 conversations across all businesses</p>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-outline-variant">
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-4 py-10 text-center text-outline">No conversations yet</td>
                </tr>
              ) : (
                data?.recentThreads.map((thread) => (
                  <tr key={thread._id} className="hover:bg-surface transition-colors">
                    <td className="px-4 py-3 font-medium text-on-surface whitespace-nowrap">{thread.businessName}</td>
                    <td className="px-4 py-3">
                      <div className="text-on-surface font-medium">{thread.leadName}</div>
                      {thread.leadPhone && (
                        <div className="text-xs text-outline">{thread.leadPhone}</div>
                      )}
                    </td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-semibold ${
                        thread.aiEnabled ? 'bg-secondary-container/40 text-on-secondary-container' : 'bg-surface-container text-outline'
                      }`}>
                    ...
                    <td className="px-4 py-3 text-outline whitespace-nowrap text-xs">
