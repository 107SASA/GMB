...
export async function GET(req: NextRequest) {
  try {
    ...
    const convertedLeads = await Lead.countDocuments({ businessId, status: 'Converted' });
    const conversionRate = totalLeads > 0 ? (convertedLeads / totalLeads) * 100 : 0;

    const aiResponseCount = await Conversation.countDocuments({ businessId, aiGenerated: true });
